...
// following element is the sum of the 4 previous, a Pentabonacci (well Cinquebonacci would probably sound a bit more italian, 
// but it would also sound really awful) with a signature of 5 elements and each following element is the sum of the 5 previous, 
// and so on.
...
function Xbonacci(signature,n){
	var xLength = signature.length;

	if (n <= xLength) {
		var nSign = [];
		for (var j = 0; j < n; j++) {
			nSign.push(signature[j]);
		}
		return nSign;
	}

	while(n > signature.length && signature.length <= n) {
		var summ = 0;

		for (var i = (signature.length - xLength); i < signature.length; i++) {
			summ = summ + signature[i];
		}
